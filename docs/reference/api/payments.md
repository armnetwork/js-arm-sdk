...
| --- | --- | --- | --- |
| `payments()` | [`payments_all`] |  | Access all payments.
| `.forAccount("address")` | [`payments_for_account`] | `string` | Pass in the address of a particular account to access its payments.|
| `.forLedger("ledgerSeq")` | [`payments_for_ledger`] | `string` | Pass in the sequence of a particular ledger to access its payments. |
| `.forTransaction("transactionHash")` | [`payments_for_transactions`] | `string` |  Pass in the hash of a particular transaction to access its payments. |
