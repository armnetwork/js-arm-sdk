...
describe("integration tests", function () {
  ...
  let server = new ArmSdk.Server(HORIZON);
  let master = ArmSdk.Keypair.random();
    ...
    server.operations().forAccount(master.publicKey()).limit(1).order('asc')
      ...
      .then(response => {
        ...
        return server.loadAccount(master.publicKey())
          .then(source => {
            let tx = new ArmSdk.TransactionBuilder(source)
              .addOperation(ArmSdk.Operation.accountMerge({
  ...
  function createNewAccount(accountId) {
    return server.loadAccount(master.publicKey())
      .then(source => {
        let tx = new ArmSdk.TransactionBuilder(source)
          .addOperation(ArmSdk.Operation.createAccount({
  ...
  describe("/transaction", function () {
    it("submits a new transaction", function (done) {
      createNewAccount(ArmSdk.Keypair.random().publicKey())
        .then(result => {
    ...
    it("submits a new transaction with error", function (done) {
      server.loadAccount(master.publicKey())
        .then(source => {
          ...
          let tx = new ArmSdk.TransactionBuilder(source)
            .addOperation(ArmSdk.Operation.createAccount({
              destination: ArmSdk.Keypair.random().publicKey(),
              startingBalance: "1"
            }))
            .build();
  ...
  describe("/accounts", function () {
    ...
    it("stream accounts", function (done) {
      ...
      let randomAccount = ArmSdk.Keypair.random();
