describe("server.js tests", function () {
  beforeEach(function () {
    this.server = new ArmSdk.Server('https://horizon-live.stellar.org:1337');
  ...
  describe('Server.constructor', function () {
    it("throws error for insecure server", function () {
      expect(() => new ArmSdk.Server('http://horizon-live.stellar.org:1337')).to.throw(/Cannot connect to insecure horizon server/);
    });

    it("allow insecure server when opts.allowHttp flag is set", function () {
      expect(() => new ArmSdk.Server('http://horizon-live.stellar.org:1337', {allowHttp: true})).to.not.throw();
    });

    it("allow insecure server when global Config.allowHttp flag is set", function () {
      ArmSdk.Config.setAllowHttp(true);
      expect(() => new ArmSdk.Server('http://horizon-live.stellar.org:1337')).to.not.throw();
    });
  });
      ...
      describe("for a non existent ledger", function () {
        it("throws a NotFoundError", function (done) {
          ...
          this.server.ledgers()
            ...
            .catch(ArmSdk.NotFoundError, function (err) {
              done();
            })
            .catch(function (err) {
              done(err);
  ...
  describe('Server.submitTransaction', function() {
    it("sends a transaction", function(done) {
      let keypair = ArmSdk.Keypair.random();
      let account = new ArmSdk.Account(keypair.publicKey(), "56199647068161");
      let transaction = new ArmSdk.TransactionBuilder(account)
  ...
  describe("Smoke tests for the rest of the builders", function() {
    ...
    describe("OrderbookCallBuilder", function() {
      ...
      it("requests the correct endpoint native/credit", function (done) {
        ...
        this.server.orderbook(ArmSdk.Asset.native(), new ArmSdk.Asset('USD', "GDVDKQFP665JAO7A2LSHNLQIUNYNAAIGJ6FYJVMG4DT3YJQQJSRBLQDG"))
          .call()
          .then(function (response) {
      ...
      it("requests the correct endpoint credit/native", function (done) {
        ...
        this.server.orderbook(new ArmSdk.Asset('USD', "GDVDKQFP665JAO7A2LSHNLQIUNYNAAIGJ6FYJVMG4DT3YJQQJSRBLQDG"), ArmSdk.Asset.native())
          .call()
          .then(function (response) {
            expect(response).to.be.deep.equal(orderBookResponse);
            done();
    ...
    describe("TradesCallBuilder", function() {
      ...
      it("trades() requests the correct endpoint for assets", function (done) {
        ...
        this.server.trades()
            .forAssetPair(ArmSdk.Asset.native(), new ArmSdk.Asset('JPY', "GBVAOIACNSB7OVUXJYC5UE2D4YK2F7A24T7EE5YOMN4CE6GCHUTOUQXM"))
            .call()
            .then(function (response) {
              expect(response.records).to.be.deep.equal(tradesResponse._embedded.records);
              done();
    ...
    describe("PathsCallBuilder", function() {
      ...
      it("requests the correct endpoint", function (done) {
        ...
        this.server.paths("GARSFJNXJIHO6ULUBK3DBYKVSIZE7SC72S5DYBCHU7DKL22UXKVD7MXP","GAEDTJ4PPEFVW5XV2S7LUXBEHNQMX5Q2GM562RJGOQG7GVCE5H3HIB4V", new ArmSdk.Asset('EUR', 'GDSBCQO34HWPGUGQSP3QBFEXVTSR2PW46UIGTHVWGWJGQKH3AFNHXHXN'), '20.0')
          .call()
          .then(function (response) {
    ...
    describe("TradeAggregationCallBuilder", function () {
      ...
      it("requests the correct endpoint native/credit", function (done) {
        ...
        this.server.tradeAggregation(ArmSdk.Asset.native(), new ArmSdk.Asset('BTC', "GATEMHCCKCY67ZUCKTROYN24ZYT5GK4EQZ65JJLDHKHRUZI3EUEKMTCH"), 1512689100000, 1512775500000, 300000)
          .call()
          .then(function (response) {
      ...
      it("requests the correct endpoint credit/native", function (done) {
        ...
        this.server.tradeAggregation(new ArmSdk.Asset('BTC', "GATEMHCCKCY67ZUCKTROYN24ZYT5GK4EQZ65JJLDHKHRUZI3EUEKMTCH"), ArmSdk.Asset.native(), 1512689100000, 1512775500000, 300000)
          .call()
          .then(function (response) {
            expect(response.records).to.be.deep.equal(tradeAggregationResponse._embedded.records);
            expect(response.next).to.be.function;
